...
class ChunkedRedisBloomFilter(BaseBloomFilter):
    ...
    def __init__(self,
        ...
        self.value_split_num = 2 if block_num <= 256 else 3
    ...
    def add(self, item: Any) -> bool:
        ...
        if not isinstance(item, str):
            item = str(item)
        redis_chunk_key = f"{self.key}:{str((int(md5(item.encode()).hexdigest()[:self.value_split_num], 16) % self.block_num))}"
    ...
    def clear(self) -> None:
        ...
        self.redis_client.delete(
            *(f"{self.key}:{str(i)}" for i in range(min(self.block_num, 4096)))
        )
