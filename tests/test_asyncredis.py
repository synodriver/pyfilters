...
class TestAsyncRedisBloomFilterResp3(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        self.redis = Redis(host=redis_addr, port=6379, db=0, password=redis_password, protocol=3)
        self.rbf = RedisBloomFilter(self.redis, "bloomfilter", 10000, 0.00001)
        self.crbf = CountRedisBloomFilter(
            self.redis, "countbloomfilter", 10000, 0.00001
        )
